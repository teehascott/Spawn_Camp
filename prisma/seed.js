...
const seed = async () => {
  try {
    await prisma.gameComment.deleteMany();
    await prisma.discComment.deleteMany();
    await prisma.gamePost.deleteMany();
    await prisma.discPost.deleteMany();
  ...
  const userResults = await Promise.allSettled([
    prisma.user.create({
      data: {
        ...
        avatar: 'https://imgur.com/Fbz7MvW',
      },
    }),
    prisma.user.create({
      data: {
        ...
        avatar: 'https://imgur.com/qbmtZFU',
      },
    }),
    prisma.user.create({
      data: {
        ...
        avatar: 'https://imgur.com/9utie05',
      },
    }),
    prisma.user.create({
      data: {
        ...
        avatar: 'https://imgur.com/HFyHcTr',
      },
    }),
    prisma.user.create({
      data: {
        ...
        avatar: 'https://imgur.com/qR2JvMt',
      },
    }),
  ]);
  ...
  const gamePostResult = await Promise.allSettled([
    ...
    prisma.gamePost.create({
     ...
     const createdGamePosts = gamePostResult
     .filter((result) => result.status === 'fulfilled')
     .map((result) => result.value);
...
const discCommentsData = [
  ...
  const discCommentResults = await Promise.allSettled(
  ...
  discCommentResults.forEach((result, index) => {
    if (result.status === 'rejected') {
      console.error(`Error creating discussion comment ${index + 1}:`, result.reason);
    }
  });
  
  const createdDiscComments = discCommentResults
    .filter((result) => result.status === 'fulfilled')
    .map((result) => result.value);
  
  if (createdDiscComments.length === 0) throw new Error('No discussion comments were created.');
  console.log('Created Discussion Comments:', createdDiscComments);  
